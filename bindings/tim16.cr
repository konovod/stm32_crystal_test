...
module TIM16
  ...
  struct CR2
    ...
    enum OIS1N : UInt8
      ...
      def self.reset_value : OIS1N
        CR2.reset_value.ois1_n
      end
    end

    # Output Idle state 1
    def ois1_n : OIS1N
      OIS1N.new!((@value >> 9) & 0x1_u32)
    end

    # Output Idle state 1
    def self.ois1_n : OIS1N
      value.ois1_n
    end

    # Output Idle state 1
    def self.ois1_n=(value : OIS1N) : OIS1N
    ...
    enum OIS1 : UInt8
      ...
      def self.reset_value : OIS1
        CR2.reset_value.ois1
      end
    end

    # Output Idle state 1
    def ois1 : OIS1
      OIS1.new!((@value >> 8) & 0x1_u32)
    end
    ...
    enum CCUS : UInt8
      ...
      WITHRISINGEDGE = 0x1_u64

      def self.reset_value : CCUS
        CR2.reset_value.ccus
      end
    end

    # Capture/compare control update              selection
    def ccus : CCUS
      CCUS.new!((@value >> 2) & 0x1_u32)
    end
    ...
    enum CCPC : UInt8
      ...
      NOTPRELOADED = 0x0_u64

      # CCxE, CCxNE and OCxM bits are preloaded
      PRELOADED = 0x1_u64

      def self.reset_value : CCPC
        CR2.reset_value.ccpc
      end
    end

    # Capture/compare preloaded              control
    def ccpc : CCPC
      CCPC.new!((@value >> 0) & 0x1_u32)
    end
  ...
  struct DIER
    ...
    enum COMIE : UInt8
      ...
      def self.reset_value : COMIE
        DIER.reset_value.comie
      end
    end

    # COM interrupt enable
    def comie : COMIE
      COMIE.new!((@value >> 5) & 0x1_u32)
    end

    # COM interrupt enable
    def self.comie : COMIE
      value.comie
    end

    # COM interrupt enable
    def self.comie=(value : COMIE) : COMIE
    ...
    enum BIE : UInt8
      ...
      def self.reset_value : BIE
        DIER.reset_value.bie
      end
    end

    # Break interrupt enable
    def bie : BIE
      BIE.new!((@value >> 7) & 0x1_u32)
    end

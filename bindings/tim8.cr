...
module TIM8
  ...
  struct OR
    ...
    def etr_adc2_rmp : UInt8
      UInt8.new!((@value >> 0) & 0x3_u32)
    end

    # TIM8_ETR_ADC2 remapping              capability
    def self.etr_adc2_rmp : UInt8
      value.etr_adc2_rmp
    end

    # TIM8_ETR_ADC2 remapping              capability
    def self.etr_adc2_rmp=(value : UInt8) : UInt8
      self.set(etr_adc2_rmp: value)
      value
    end

    # TIM8_ETR_ADC3 remapping              capability
    def etr_adc3_rmp : UInt8
      UInt8.new!((@value >> 2) & 0x3_u32)
    end

    # TIM8_ETR_ADC3 remapping              capability
    def self.etr_adc3_rmp : UInt8
      value.etr_adc3_rmp
    end

    # TIM8_ETR_ADC3 remapping              capability
    def self.etr_adc3_rmp=(value : UInt8) : UInt8
      self.set(etr_adc3_rmp: value)
      value
    end
    ...
    def copy_with(
      *,
      ...
      etr_adc2_rmp : UInt8? = nil,

      etr_adc3_rmp : UInt8? = nil
    ) : self
      value = @value

      unless etr_adc2_rmp.nil?
        value = (value & 0xfffffffc_u32) |
                UInt32.new!(etr_adc2_rmp.to_int).&(0x3_u32) << 0
      end

      unless etr_adc3_rmp.nil?
        value = (value & 0xfffffff3_u32) |
                UInt32.new!(etr_adc3_rmp.to_int).&(0x3_u32) << 2
      end
    ...
    def self.set(
      *,
      ...
      self.value = self.value.copy_with(
        etr_adc2_rmp: etr_adc2_rmp,
        etr_adc3_rmp: etr_adc3_rmp,
      )
    end
  end # struct
